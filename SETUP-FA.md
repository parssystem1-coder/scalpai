...
> ⚠️ **به‌روزرسانی (فاز A):** راه‌حل توضیح‌داده‌شده در این بخش روی ماشین شخصی اجرا شد
> ولی فایل‌های ساخته‌شده **هرگز کامیت نشدند** — پوشهٔ `build/` تا فاز A در مخزن غایب بود
> و بیلد ویندوز روی هر کلون تازه شکست می‌خورد. راه‌حل نهایی و پایدار حالا
> `scripts/generate-build-assets.cjs` است که دارایی‌ها را از روی `public/icon.png`
> بازتولید می‌کند و با `scripts/test-build-assets.cjs` در CI نگهبانی می‌شود.
> جزئیات: `docs/فاز-A-گزارش-اجرا.md`
...
| در تنظیمات، وضعیت رمزنگاری «غیرفعال» نشان داده می‌شود | `safeStorage` سیستم‌عامل در دسترس نیست (روی ویندوز نادر است؛ عمدتاً لینوکسِ بدون keyring) | بخش «۶٫۱ — رمزنگاری و کلید» را ببینید |
| پس از تعویض کاربر ویندوز یا کپی پوشهٔ داده، برنامه بالا می‌آید ولی داده‌ها دیده نمی‌شوند | کلید رمزنگاری به حساب کاربری ویندوز گره خورده و روی حساب/دستگاه جدید باز نمی‌شود (`unwrapped-error`) | بخش «۶٫۱ — رمزنگاری و کلید» را ببینید |

---

### ۶٫۱ — رمزنگاری و کلید (خواندنش قبل از استقرار در کلینیک واجب است)

**کلید کجاست؟** برنامه هنگام اولین اجرا یک کلید تصادفی می‌سازد و آن را با
`safeStorage` ویندوز (سازوکار DPAPI) رمز کرده و در فایل `scalpai.key` داخل پوشهٔ
دادهٔ برنامه نگه می‌دارد. این کلید **به حساب کاربری ویندوز شما گره خورده است**.
یعنی اگر کسی کل پوشهٔ داده را بدزدد، بدون نشست همان کاربر ویندوز نمی‌تواند
دیتابیس و تصاویر را باز کند (`electron/dek.cjs`).

**دو سناریوی مهم که باید بدانید:**

۱. **تعویض حساب کاربری ویندوز یا انتقال پوشهٔ داده به کامپیوتر دیگر** — کلید
   روی حساب جدید باز نمی‌شود. برنامه عمداً کرش نمی‌کند و بالا می‌آید، ولی لایهٔ
   رمز غیرفعال گزارش می‌شود و دادهٔ رمزشدهٔ قبلی قابل خواندن نیست.
   **راه درست انتقال بین دستگاه‌ها: فقط از «پشتیبان با پسورد» استفاده کنید**،
   نه کپی دستی پوشهٔ داده. (از فاز ۱ به بعد، وقتی رمزنگاری فعال باشد پسورد
   پشتیبان اجباری است — دقیقاً به همین دلیل.)

۲. **`safeStorage` در دسترس نیست** (روی ویندوز نادر است) — برنامه آگاهانه
   بدون رمزنگاری ادامه می‌دهد («fail-open») چون قفل‌شدن دادهٔ کلینیک بدترین
   حالت ممکن است. این وضعیت **پنهان نمی‌ماند**: در تنظیمات → بخش رمزنگاری با
   نشان زرد/قرمز نمایش داده می‌شود. اگر چنین چیزی دیدید، تا رفع آن دادهٔ
   بالینی واقعی وارد نکنید.

**کلید بازیابی:** در تنظیمات → بخش رمزنگاری دکمه‌ای هست که پس از وارد کردن
دوبارهٔ پسورد برنامه، کلید را یک‌بار نشان می‌دهد. آن را در جای امن (نه کنار
خود کامپیوتر) یادداشت کنید. هر بار نمایش این کلید در ردپای حسابرسی ثبت می‌شود
و در تنظیمات → تب «ردپای حسابرسی» قابل مشاهده است.
