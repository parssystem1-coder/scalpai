...
**F15 - `REQUIRED_GATES` is in sync; the enforcement hole is elsewhere.** `tools/ci/gate-report.ts` lists 25 gates and every one is recorded by a `run-gate.sh` step in `ci.yml` - no drift. The real gap: **no gate exists for accessibility, locale parity, dashboard integration or performance marks**, so criteria 9-12 have no path to enforcement even after the code is fixed. And `ci.yml` cannot grant itself required-check status; that is the repo ruleset (P4-B03).
