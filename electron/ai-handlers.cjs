...
function createAiHandlers(net) {
  return {
    async analyze(params) {
      ...
      // فاز ۳٫۳ — تلاش مجدد فقط برای خطاهای گذرا (5xx/شبکه)؛ نه برای
      // کلید نامعتبر یا rate-limit که تکرارشان بی‌فایده و پرهزینه است.
      let result = null;
      let retryCount = 0;
      for (let attempt = 0; attempt <= AI_MAX_TRANSIENT_RETRIES; attempt++) {
        result = await netPostJson(net, url, headers, body, AI_ANALYZE_TIMEOUT_MS, requestId || null);
        if (result.aborted || result.ok) break;
        if (attempt >= AI_MAX_TRANSIENT_RETRIES) break;
        const msg = extractErrorMessage(result.status, result.data);
        if (!isTransientAiError(result.status, msg)) break;
      ...
      if (!result.ok) {
        ...
        return { success: false, status: result.status, error, retryCount };
      }
      ...
      return {
        ...
        responseModelId: extractResponseModelId(config.provider, result.data) || undefined,
        retryCount,
      };
    },
