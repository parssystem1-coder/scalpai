...
async function applyPatientCreate(ctx: PushCtx, env: MutationEnvelope): Promise<PushItemResult> {
  const row = await ctx.tx.insert(patients).values({
...
async function applyPatientUpdate(ctx: PushCtx, env: MutationEnvelope): Promise<PushItemResult> {
  ...
  const merge = mergeFieldLww({ ...live[0]!, updatedAt: (live[0]!.updatedAt as unknown as Date).toISOString() }, env as unknown as MutationEnvelope<Record<string, unknown>>);
  if (merge.action === "rejected-stale-base") return { clientMutationId: env.clientMutationId, status: "rejected", reason: "stale baseVersion" };
...
async function applyPlanCreate(ctx: PushCtx, env: MutationEnvelope): Promise<PushItemResult> {
  ...
  const row = await ctx.tx.insert(treatmentPlans).values({
    clinicId: ctx.clinicId,
    patientId,
...
async function applyPlanUpdate(ctx: PushCtx, env: MutationEnvelope): Promise<PushItemResult> {
  ...
  if (merge.action === "rejected-stale-base") return { clientMutationId: env.clientMutationId, status: "rejected", reason: "stale baseVersion" };
  const fields: Record<string, unknown> = {};
  for (const [k, v] of Object.entries(merge.fields)) if (SAFE_PLAN_FIELDS.has(k)) fields[k] = v;
  if (Object.keys(fields).length === 0) return { clientMutationId: env.clientMutationId, status: "applied" };
...
export async function processPushBatch(ctx: PushCtx, envelopes: MutationEnvelope[]): Promise<PushItemResult[]> {
  ...
  const existing = await ctx.tx.select({ clientMutationId: mutations.clientMutationId }).from(mutations).where(and(eq(mutations.clinicId, ctx.clinicId), inArray(mutations.clientMutationId, ids)));
  ...
  for (const env of envelopes) {
    ...
    if (forbidden) {
      results.push({ clientMutationId: env.clientMutationId, status: "rejected", reason: `field '${forbidden}' carries plaintext PHI — encrypt it before syncing` });
      continue;
    }
    ...
    await ctx.tx.insert(mutations).values({
      ...
      entity: env.entity,
      op: env.op,
      payload: canonicalObject<object>({ ...redactPhiPayload(env.payload), _fields: payloadFieldNames(env.payload) }),
    });
    results.push(res);
  }
  return results;
...
export async function pullSince(tx: Tx, clinicId: string, sinceSeq: number, limit: number): Promise<{ items: Array<{ id: number; entity: string; op: string; payload: unknown; serverSeq: number; at: Date }>; nextSeq: number }> {
  const items = await tx.select({ id: mutations.id, entity: mutations.entity, op: mutations.op, payload: mutations.payload, serverSeq: mutations.serverSeq, at: mutations.at }).from(mutations).where(and(eq(mutations.clinicId, clinicId), gt(mutations.serverSeq, sinceSeq))).orderBy(mutations.serverSeq).limit(limit);
