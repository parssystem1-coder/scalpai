...
describe("listDueClinics", () => {
  it("maps snake_case rows to camelCase", async () => {
    ...
    const result = await listDueClinics(tx as any, 10);
  ...
  it("returns empty array for no due clinics", async () => {
    const tx = mockTx([]);
    const result = await listDueClinics(tx as any);
    expect(result).toEqual([]);
  });
  it("defaults limit to 200", async () => {
    const tx = mockTx([]);
    await listDueClinics(tx as any);
    expect(tx.execute).toHaveBeenCalled();
  });
  it("throws for limit below 1", async () => {
    const tx = mockTx([]);
    await expect(listDueClinics(tx as any, 0)).rejects.toThrow();
  });
  it("throws for limit above DUE_CLINIC_LIMIT_MAX", async () => {
    const tx = mockTx([]);
    await expect(listDueClinics(tx as any, DUE_CLINIC_LIMIT_MAX + 1)).rejects.toThrow();
  });
  it("throws for non-integer limit", async () => {
    const tx = mockTx([]);
    await expect(listDueClinics(tx as any, 1.5)).rejects.toThrow();
  });
  it("accepts limit of exactly 1", async () => {
    const tx = mockTx([]);
    await expect(listDueClinics(tx as any, 1)).resolves.toEqual([]);
  });
  it("accepts limit of exactly DUE_CLINIC_LIMIT_MAX", async () => {
    const tx = mockTx([]);
    await expect(listDueClinics(tx as any, DUE_CLINIC_LIMIT_MAX)).resolves.toEqual([]);
  });
  it("handles multiple clinics", async () => {
    ...
    const result = await listDueClinics(tx as any, 100);
