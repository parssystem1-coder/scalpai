...
export const InboxPage: React.FC = () => {
  ...
  const load = useCallback(async (nextOffset: number) => {
    setLoading(true);
    try {
      const result = await apiFetch<InboxResponse>(`/aftercare/inbox?limit=20&offset=${nextOffset}`);
      const incoming = result.items ?? result.data ?? [];
      setMessages((current) => nextOffset === 0 ? incoming : [...current, ...incoming]);
  ...
  return <main dir="rtl" className="min-h-screen bg-[oklch(85%_0.03_28)] p-4 md:p-8" aria-labelledby="inbox-title">
    <div className="mx-auto max-w-7xl"><header className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h1 id="inbox-title" className="text-2xl font-black">{t("inbox.title")}</h1><p className="mt-1 text-sm opacity-65">{t("inbox.subtitle")}</p></div><input aria-label={t("inbox.searchLabel")} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("inbox.searchPlaceholder")} className="rounded-xl border border-black/10 bg-white px-4 py-3 text-sm" /></header>
      <div className="grid overflow-hidden rounded-2xl border border-white/70 bg-white/40 shadow-xl md:grid-cols-[minmax(260px,34%)_1fr]"><aside className="max-h-[70vh] overflow-y-auto" aria-label={t("inbox.conversations")}>
        {filtered.length === 0 && !loading && <p className="p-8 text-center text-sm opacity-60">{t("inbox.noMessages")}</p>}
        {filtered.map((message) => <MessageCard key={message.id} message={message} selected={message.id === selectedId} onSelect={(id) => void select(id)} />)}
        {hasMore && <button type="button" onClick={() => void load(offset + 20)} disabled={loading} className="w-full p-4 text-sm font-bold">{loading ? t("inbox.loading") : t("inbox.loadMore")}</button>}
      </aside><MessageThread message={selected} body={body} reply={reply} onReplyChange={setReply} onReply={() => void sendReply()} sending={sending} /></div>
    </div>
  </main>;
};
