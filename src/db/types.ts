...
export interface AIAnalysisResult {
  ...
  /**
   * فاز ۰٫۲ — تناقض‌های داخلی پاسخ AI (امتیاز عددی در برابر تشخیص گزارش‌شده).
   * فقط اطلاع‌رسانی است؛ نتیجه به‌خاطر آن رد یا اصلاح نمی‌شود.
   */
  consistencyConflicts?: import('../lib/aiResultConsistency').ConsistencyConflict[];
  /**
   * فاز ۳٫۲ — شناسهٔ مدلی که واقعاً پاسخ داده. روترها (مثل OpenRouter) ممکن است
   * درخواست را به مدل دیگری بفرستند؛ بدون این فیلد نمی‌توان کیفیت را به مدل نسبت داد.
   */
  responseModelId?: string;
  /**
   * فاز ۳٫۵ — نشانه‌هایی که این پاسخ ممکن است بدون دیدن واقعی تصویر تولید شده باشد.
   * فقط هشدار است؛ نتیجه رد نمی‌شود.
   */
  genericResponseWarning?: string[];
}
...
export interface OfflineAnalysisResult {
   ...
   */
  /**
   * فاز ۴٫۳ — سنجش شباهت ورودی به توزیع دادهٔ آموزشی مدل محلی.
...
export interface TrainingSample {
  ...
  /**
   * فاز ۳٫۱ — snapshot برچسب اولیهٔ AI آنلاین، پیش از آنکه متخصص آن را تصحیح کند.
   * فقط یک‌بار (اولین تصحیح) نوشته می‌شود و بعد از آن دست‌نخورده می‌ماند، تا
   * مبنای سنجش «توافق AI با متخصص» پایدار بماند.
   */
  originalAiLabel?: TrainingSampleLabel;
  /** زمان ثبت snapshot اولیهٔ AI */
  originalAiLabelAt?: string;
}
...
export interface LocalModelMetadata {
  ...
  /** فاز ۲٫۱ — F1 ماکرو روی holdout؛ صادقانه‌تر از میکرو در دیتاست نامتوازن */
  holdoutMacroF1?: number;
  /** فاز ۲٫۱ — گزارش per-class روی holdout */
  holdoutPerClass?: import('../lib/mlEvaluation').ClassificationSummary;
  /** فاز ۲٫۲ — support هر برچسب در استخر آموزشی */
  labelSupport?: import('../lib/mlEvaluation').LabelSupport[];
  /** فاز ۲٫۲ — برچسب‌هایی که مدل روی آن‌ها اظهارنظر نمی‌کند (support ناکافی) */
  suppressedLabels?: string[];
  /** فاز ۲٫۴ — تجمیع چند اجرای holdout */
  repeatedHoldout?: {
