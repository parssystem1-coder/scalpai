...
import { findPriorAnalysisForImage, type DuplicateAnalysisInfo } from '../../lib/imageDedup';
import { detectGenericAiResponse } from '../../lib/aiResultConsistency';
...
export function useAISession() {
  ...
  /**
   * فاز ۰٫۴ — اگر همین تصویر قبلاً تحلیل شده، قبل از مصرف سهمیهٔ API بپرس.
   * تصمیم با کاربر است؛ ممکن است عمداً بخواهد با مدل دیگری دوباره تحلیل کند.
   */
  const [duplicateWarning, setDuplicateWarning] = useState<DuplicateAnalysisInfo | null>(null);

  const analyzeWithGemini = async (options: { ignoreDuplicate?: boolean } = {}) => {
    if (!selectedImage) {
      setError(t('selectImageFirst'));
      return;
    }
    if (!options.ignoreDuplicate) {
      const prior = findPriorAnalysisForImage(analyses, {
        clientId: selectedClient,
        galleryItemId: selectedImage.id,
        type: 'ai',
      });
      if (prior) {
        setDuplicateWarning(prior);
        return;
      }
    }
    ...
    try {
      ...
      // فاز ۳٫۵ — هشدار در صورت پاسخ مشکوک به «ندیدن تصویر»
      const genericSignal = detectGenericAiResponse({
      ...
      if (genericSignal.suspicious) {
        console.warn('Suspicious generic AI response:', genericSignal.reasonsEn);
      }

      const analysisResult: AIAnalysisResult = {
        ...parsed,
        responseModelId: apiResult.responseModelId,
        genericResponseWarning: genericSignal.suspicious
          ? (lang === 'fa' ? genericSignal.reasonsFa : genericSignal.reasonsEn)
          : undefined,
        acquisitionContext,
        questionnaireContext,
  ...
  return {
    ...
    selectImage,
    analyzeWithGemini,
    /** فاز ۰٫۴ — تحلیل تکراری تشخیص داده شد؛ منتظر تأیید کاربر */
    duplicateWarning,
    dismissDuplicateWarning: () => setDuplicateWarning(null),
    /** ادامهٔ تحلیل با وجود تکراری‌بودن (تصمیم آگاهانهٔ کاربر) */
    analyzeAnyway: () => analyzeWithGemini({ ignoreDuplicate: true }),
    cancelAnalysis,
    loadAnalysisForView,
    handleDelete,
    resetForm,
