  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      ...
      <div className="space-y-4">
        ...
        {/* فاز ۰٫۴ — این تصویر قبلاً تحلیل شده؛ قبل از مصرف سهمیهٔ API تأیید بگیر */}
        {s.duplicateWarning && !s.analyzing && (
          <div className="p-3 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-200 text-sm space-y-2">
            <div className="flex items-start gap-2">
              <AlertCircle size={18} className="flex-shrink-0 mt-0.5" />
              <span>
                {lang === 'fa'
                  ? `این تصویر قبلاً ${s.duplicateWarning.count} بار تحلیل شده است (آخرین بار: ${new Date(s.duplicateWarning.createdAt).toLocaleString('fa-IR')}). تحلیل دوباره سهمیهٔ سرویس هوش مصنوعی را مصرف می‌کند.`
                  : `This image has already been analyzed ${s.duplicateWarning.count} time(s) (last: ${new Date(s.duplicateWarning.createdAt).toLocaleString('en-US')}). Re-analyzing consumes AI service quota.`}
              </span>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={s.analyzeAnyway}
                className="px-3 py-1.5 rounded-lg bg-amber-500/25 hover:bg-amber-500/35 text-xs font-medium"
              >
                {lang === 'fa' ? 'با این حال دوباره تحلیل کن' : 'Analyze again anyway'}
              </button>
              <button
                type="button"
                onClick={s.dismissDuplicateWarning}
