...
interface Props {
  ...
  /** فاز ۰٫۱ — نتیجهٔ گیت مقایسه با مدل قبلی */
  retrainNotice?: { replaced: boolean; reason: string } | null;
  /** فاز ۰٫۱ — آموزش مجدد با جایگزینی اجباری (وقتی گیت مدل را رد کرده) */
  onForceTrain?: () => void;
  /** فاز ۰٫۵ — بازگردانی به نسخهٔ پشتیبان قبلی */
  canRollback?: boolean;
  onRollback?: () => void;
  ...
  return (
    <div className="rounded-2xl bg-white/5 border border-white/10 p-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* فاز ۰٫۵ — بازگردانی مدل به نسخهٔ پشتیبان قبلی */}
        {canRollback && onRollback && (
          <button
            onClick={onRollback}
            disabled={rollbackBusy || training}
            title={pick(
              'بازگردانی وزن‌های مدل به نسخهٔ قبل از آخرین آموزش',
              'Restore model weights to the version before the last training',
            )}
            className="px-3 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 disabled:opacity-40 text-amber-300 text-sm flex items-center gap-1.5"
          >
            <RotateCcw size={14} /> {pick('بازگردانی نسخهٔ قبلی', 'Rollback')}
          </button>
        )}
      ...
      {/* فاز ۰٫۱ — نتیجهٔ گیت champion/challenger */}
      {retrainNotice && (
        <div
          className={`mt-3 rounded-xl border p-3 text-xs space-y-2 ${
            retrainNotice.replaced
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <span className="whitespace-pre-wrap">{retrainNotice.reason}</span>
          </div>
          {!retrainNotice.replaced && onForceTrain && (
            <button
              onClick={onForceTrain}
              disabled={training}
              className="px-3 py-1.5 rounded-lg bg-amber-500/25 hover:bg-amber-500/35 disabled:opacity-40 font-medium"
            >
              {pick('با وجود افت، جایگزین کن (آموزش مجدد اجباری)', 'Replace anyway (forced retrain)')}
            </button>
          )}
        </div>
      )}
