...
function buildModel(inputSize: number): tf.LayersModel {
  ...
  const multiTaskLoss = (yTrue: tf.Tensor, yPred: tf.Tensor) =>
    tf.tidy(() => {
      ...
      const eps = 1e-7;
      const bce = tf.mean(tf.neg(tf.add(
        tf.mul(obsTrue, tf.log(tf.add(obsPred, eps))),
        tf.mul(tf.sub(1, obsTrue), tf.log(tf.add(tf.sub(1, obsPred), eps))),
      )));
...
async function fitOnSplit(
  ...
  const model = buildModel(trainX[0].length);
  ...
  try {
    ...
    return {
      ...
      metrics: {
        ...
        holdoutObsF1: holdRows.length ? obsF1(holdRows.map(r => r.y), holdPred) : undefined,
      },
    };
  } catch (err) {
...
export async function predictWithLocalModel(
  ...
  let vector = featureVectorToArray(features);
  ...
  const observations = OBSERVATION_IDS.filter((_, i) => {
    const v = values[SCORE_COUNT + i] ?? 0;
    return v >= OBS_THRESHOLD;
  });
