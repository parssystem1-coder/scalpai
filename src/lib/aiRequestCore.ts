...
/** فاز ۳٫۳ — حداکثر تلاش مجدد برای خطای گذرا (کم نگه داشته شده تا کاربر معطل نشود) */
export const AI_MAX_TRANSIENT_RETRIES = 2;
...
/**
 * فاز ۳٫۲ — شناسهٔ مدلی که واقعاً پاسخ داده.
 *
 * مهم است چون روترهایی مثل OpenRouter می‌توانند درخواست را به مدل دیگری
 * (گاهی بدون قابلیت vision) بفرستند. بدون ثبت این مقدار، هیچ راهی نداریم
 * بفهمیم نتیجهٔ ضعیف از مدل بوده یا از پرامپت.
 */
export function extractResponseModelId(provider: string, data: unknown): string | null {
  if (provider === 'gemini') {
    const d = data as { modelVersion?: string; model?: string };
    return d?.modelVersion || d?.model || null;
  }
  const d = data as { model?: string };
  return typeof d?.model === 'string' && d.model ? d.model : null;
}

/**
 * فاز ۳٫۳ — آیا خطا گذراست و تلاش مجدد منطقی است؟
 * عمداً 401/403/404 و خطاهای سهمیه را شامل نمی‌شود: تکرار آن‌ها فقط
 * سهمیه می‌سوزاند و کاربر را منتظر نگه می‌دارد بدون اینکه نتیجه عوض شود.
 */
export function isTransientAiError(status?: number, message?: string): boolean {
...
/** فاز ۳٫۳ — تأخیر پس‌رفت نمایی با jitter برای جلوگیری از هم‌زمانی درخواست‌ها */
export function backoffDelayMs(attempt: number, baseMs = 700, maxMs = 6000): number {
  const exp = Math.min(maxMs, baseMs * 2 ** Math.max(0, attempt));
  // jitter کامل: بازهٔ [exp/2, exp]
